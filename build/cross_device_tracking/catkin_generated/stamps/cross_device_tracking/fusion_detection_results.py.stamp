...
LastEditTime: 2023-07-04 15:45:56
...
class MsgTrans(object):
    ...
    def __init__(
        ...
        self.translation_vector_3 = np.array(
            [[-116.137], [-151.939], [-6.889236450195312]])
        ...
        self.translation_vector_25 = np.array(
            [[-297.244], [-161.664], [-6.922]])
    ...
    def callback(self, msg_data1, msg_data2):
        ...
        print("lidar1时间戳:{0},lidar2时间戳:{1}".format(self.timestamp1, self.timestamp2))
        # 得到的结果是Object[]，也就是Objects类型的
        self.obj = self.fusion_detection_result(det_list1=self.obj_list1, det_list2=self.obj_list2)
        # for obj in self.obj:
        ...
        # 将检测结果修改为融合后的部分，其余的部分与 lidar 1 保持一直
        rsPerceptionMsg.lidarframe.objects.objects = self.obj
    ...
    return {*}
    '''
    def fusion_detection_result(self, det_list1, det_list2):

        # 第一步，将二者都变换到enu 坐标系  速腾已经变换过了
    ...
    def toenu(self, list, rotation_martix, translation_vector):
        for obj in list:
            point = np.array(
                [obj.coreinfo.center.x.data, obj.coreinfo.center.y.data, obj.coreinfo.center.z.data])
            transformed_point = self.transform_point(
                point, rotation_martix, translation_vector)
    ...
    def transform_point(self, point, rotation_matrix, translation_vector):
        ...
        transformation_matrix = np.vstack(
            (np.hstack((rotation_matrix, translation_vector)), [0, 0, 0, 1]))
        # print("*********************************************************")
