...
VisualizeDetectedObjects::VisualizeDetectedObjects()
    ...
    , label_height_(1.0) {
		...
		private_nh_.param<double>("marker_display_duration", marker_display_duration_, 1);
		...
		subscriber_detected_objects_ =
		    node_handle_.subscribe(object_src_topic, 1, &VisualizeDetectedObjects::DetectedObjectsCallback, this);
...
void VisualizeDetectedObjects::DetectedObjectsCallback(const autoware_msgs::DetectedObjectArray& in_objects) {
		...
		visualization_markers.markers.insert(visualization_markers.markers.end(), label_markers.markers.begin(), label_markers.markers.end());
		visualization_markers.markers.insert(visualization_markers.markers.end(), arrow_markers.markers.begin(), arrow_markers.markers.end());
		visualization_markers.markers.insert(visualization_markers.markers.end(), polygon_hulls.markers.begin(), polygon_hulls.markers.end());
		visualization_markers.markers.insert(visualization_markers.markers.end(), bounding_boxes.markers.begin(), bounding_boxes.markers.end());
		visualization_markers.markers.insert(visualization_markers.markers.end(), object_models.markers.begin(), object_models.markers.end());
		visualization_markers.markers.insert(visualization_markers.markers.end(), centroid_markers.markers.begin(), centroid_markers.markers.end());

		publisher_markers_.publish(visualization_markers);
		// std::cout << visualization_markers << std::endl;
}
		...
		for (auto const& object : in_objects.objects) {
				if (IsObjectValid(object) &&
				    ...
				    (object.dimensions.x + object.dimensions.y + object.dimensions.z) < object_max_linear_size_) {
						...
						} else if (object.label == "people") {
								model.mesh_resource = "package://detected_objects_visualizer/models/person.dae";
						} else if (object.label == "cyclist" || object.label == "bike") {
		...
		for (auto const& object : in_objects.objects) {
				if (IsObjectValid(object) && object.pose_reliable) {
						...
						if (abs(velocity) >= arrow_speed_threshold_) {
								...
								tf::Quaternion q(object.pose.orientation.x, object.pose.orientation.y, object.pose.orientation.z, object.pose.orientation.w);
		...
		for (auto const& object : in_objects.objects) {
				if (IsObjectValid(object)) {
						...
						if (!object.label.empty() && object.label != "unknown")
								label_marker.text = object.label + " " + std::__cxx11::to_string(object.id) + " "; // Object Class if available
